...
public class BatchWorkflow {
	...
	// this needs to make sure we convert the data in case the data structures
	// are different.
	...
	private Pail shred() throws IOException {
		...
		JavaPairRDD<Text, Data> hadoopFile = jsc.hadoopFile(source.getInstanceRoot(),
				SequenceFilePailDataInputFormat.class, Text.class, Data.class, 0);
	...
	public void normalizeURLs() throws IOException {
		...
		JavaPairRDD<Text, Data> hadoopFile = jsc.hadoopFile(masterDataset.getInstanceRoot(),
				SequenceFilePailDataInputFormat.class, Text.class, Data.class, 0);
	...
	/*
	 * Normalizing the user ID's. User ID's belonging to the same person are
	 * marked as equivs
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void normalizeUserIds() throws IOException {
		...
		JavaSparkContext jsc = JavaSparkContext.fromSparkContext(SparkContext.getOrCreate(conf));
		JavaPairRDD<Text, Data> hadoopFile = jsc.hadoopFile(equivs.getInstanceRoot(),
				SequenceFilePailDataInputFormat.class, Text.class, Data.class, 0);
		...
		Pail pageviewsPail = new Pail(Mode.SPARK, "/tmp/swa/normalized_urls")
				.getSubPail(DataUnit._Fields.PAGE_VIEW.getThriftFieldId());
		JavaPairRDD<Text, Data> pageviews = jsc.hadoopFile(pageviewsPail.getInstanceRoot(),
				SequenceFilePailDataInputFormat.class, Text.class, Data.class, 0);
		...
		JavaPairRDD<PersonID, Tuple2<Data, Optional<PersonID>>> leftOuterJoin = pageviewsPair
				.leftOuterJoin(newIdsPerson);
	...
	private JavaRDD<Tuple2<PersonID, PersonID>> runUserIdNormalizationIteration(JavaSparkContext jsc, int i) {
		...
		JavaRDD<Tuple2<PersonID, PersonID>> newEdgeSet = iteration
				.map(f -> new Tuple2<PersonID, PersonID>(f._1(), f._2())).distinct();
		newEdgeSet.saveAsObjectFile(output);

		JavaRDD<Tuple2<PersonID, PersonID>> progressEdges = iteration.filter(f -> f._3().equals(Boolean.TRUE))
				.map(f -> {
	...
	public void deduplicatePageviews() throws IOException {
		...
		JavaSparkContext jsc = JavaSparkContext.fromSparkContext(SparkContext.getOrCreate(conf));
		JavaPairRDD<Text, Data> pageviews = jsc.hadoopFile(source.getInstanceRoot(),
				SequenceFilePailDataInputFormat.class, Text.class, Data.class, 0);
	...
	public JavaRDD<Tuple4<String, String, Integer, Long>> pageviewBatchView() throws IOException {
		...
		JavaSparkContext jsc = JavaSparkContext.fromSparkContext(SparkContext.getOrCreate(conf));
		JavaPairRDD<Text, Data> pageviews = jsc.hadoopFile(sourcePail.getInstanceRoot(),
				SequenceFilePailDataInputFormat.class, Text.class, Data.class, 0);

		// hourly rollup
		// we could just return a Tuple2 for our calculation but to follow the
		// code from the book we do that in the next step
		...
		JavaPairRDD<Tuple2<String, Integer>, Long> hourlyRollupPairs = visits.mapToPair(f -> new Tuple2<>(f, 1l))
				.reduceByKey((v1, v2) -> v1 + v2);
		JavaRDD<Tuple3<String, Integer, Long>> hourlyRollup = hourlyRollupPairs
				.map(f -> new Tuple3<>(f._1._1, f._1._2, f._2));
		...
		JavaRDD<Tuple4<String, String, Integer, Long>> map2 = granularities
				.map(f -> new Tuple4<>(f._1._3(), f._1._1(), f._1._2(), f._2)).sortBy(f -> f._2(), true, 0);
